...
class AnchorsLayer(layers.Layer):
...
def fpn_classifier_graph(rois, feature_maps, image_meta,
    ...
    # a = [rois, image_meta] + feature_maps
    # print(a.shape)
    ...
    s = tf.shape(x)
    mrcnn_bbox = tf.reshape(x,(s[0],s[1], num_classes, 4))
    mrcnn_bbox = layers.Permute((1,2,3),name="mrcnn_bbox")(mrcnn_bbox)
...
def mrcnn_bbox_loss_graph(target_bbox, target_class_ids, pred_bbox):
    ...
    # Smooth-L1 Loss
    loss = tf.switch(tf.shape(target_bbox)[0] > 0,
...
class MaskRCNN():
    ...
    def build(self, mode, config):
        ...
        input_image = layers.Input(
            shape=[256, 256, config.IMAGE_SHAPE[2]], name="input_image")
        ...
        if mode == "training":
            ...
            gt_boxes=  (input_gt_boxes- shift)/scale
...
class Config(object):
    ...
    NAME = 'Mask-RCNN'  # Override in sub-classes
...
model = MaskRCNN('inference',Config(),'./').keras_model
model.summary()
for ou in model.output:
    print(ou.name)
# model.get_layer("mrcnn_bbox")
