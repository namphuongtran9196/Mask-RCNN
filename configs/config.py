...
"""
Mask R-CNN
Base Configurations class base on https://github.com/matterport/Mask_RCNN/blob/master/mrcnn/config.py
...
class Config(object):
    ...
    def __init__(self):
        ...
        self.NAME = 'Mask-RCNN'  # Override in sub-classes
        ...
        # Because the steps_per_epoch is not matched with the size of the training set,
        # We use STEPS = EPOCHS * STEPS_PER_EPOCH and logging only the step.
        # For example, with EPOCHS = 5 and STEPS_PER_EPOCH = 10, we will have STEPS = 5 * 10 = 50 
        self.STEPS = 100000
        
        # When you want to save the best weights only, set this to True.
        # Requires providing a validation dataset in training phase
        ...
        self.NUM_CLASSES = 1  + 1 # adding 1 for background
    ...
    @property
    def IMAGES_PER_GPU(self):
        return self._IMAGES_PER_GPU
    @IMAGES_PER_GPU.setter
    def IMAGES_PER_GPU(self,value):
        self._IMAGES_PER_GPU = value
        self.BATCH_SIZE = self._IMAGES_PER_GPU * self.GPU_COUNT
    ...
    def GPU_COUNT(self,value):
        ...
        try:
            self.BATCH_SIZE = self._GPU_COUNT * self.IMAGES_PER_GPU 
        except:
            self.BATCH_SIZE = self._GPU_COUNT * 1
    ...
    def NUM_CLASSES(self,value):
        self._NUM_CLASSES = value +1 # add background class
        self.IMAGE_META_SIZE = 1 + 3 + 3 + 4 + 1 + self._NUM_CLASSES
    
    @property
    def IMAGE_RESIZE_MODE(self):
        return self._IMAGE_RESIZE_MODE
    
    @IMAGE_RESIZE_MODE.setter
    def IMAGE_RESIZE_MODE(self,mode):
        self._IMAGE_RESIZE_MODE = mode
        if self._IMAGE_RESIZE_MODE == "crop":
            self.IMAGE_SHAPE = np.array([self.IMAGE_MIN_DIM, self.IMAGE_MIN_DIM,
                self.IMAGE_CHANNEL_COUNT])
        else:
            self.IMAGE_SHAPE = np.array([self.IMAGE_MAX_DIM, self.IMAGE_MAX_DIM,
