...
def rpn_class_loss_graph(rpn_match, rpn_class_logits):
    ...
    # loss =  tf.where(tf.size(loss)> 0,loss,tf.constant(0.0))
...
def mrcnn_bbox_loss_graph(target_bbox, target_class_ids, pred_bbox):
    ...
    return tf.switch_case(tf.size(loss), {0: return_zeros}, default= return_loss)
    # loss = tf.where(tf.shape(target_bbox)[0] > 0,loss,tf.constant(0.0))
    # loss = tf.reduce_mean(loss)
    # return loss
...
def mrcnn_mask_loss_graph(target_masks, target_class_ids, pred_masks):
    ...
    return tf.switch_case(tf.size(loss), {0: return_zeros}, default= return_loss)
    # loss = tf.where(tf.size(y_true)> 0,,tf.constant(0.0))
    # loss = tf.reduce_mean(loss)
    # return loss