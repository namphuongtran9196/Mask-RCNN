...
class AnchorsLayer(layers.Layer):
    ...
    def get_config(self):
        base_config = super(AnchorsLayer, self).get_config()
        config = { 'upscale_factor': self.upscale_factor, }
        return dict(list(base_config.items()) + list(config.items()))
get_custom_objects().update({'AnchorsLayer':AnchorsLayer})

class LayerIdentity(layers.Reshape):
    def __init__(self, **kwargs):
        super(LayerIdentity, self).__init__(**kwargs)
...
def fpn_classifier_graph(rois, feature_maps, image_meta,
    ...
    # Set name for layer lambda tensorflow
